...
export function ArticleContent({ slug, isDark }: ArticleContentProps) {
  if (slug === "trails-legal-case-study") {
    return (
      <div className={`w-full overflow-y-auto max-h-[calc(100vh-200px)] px-6 animate-in fade-in duration-500 ${isDark ? 'text-white' : 'text-black'}`}>
        {/* Hero Title */}
        <h1 className="text-xl font-semibold mb-2 text-left font-helvetica">
          Modern Interfaces for an Ancient Industry: My Approach to Legal UX
        </h1>
        <span className={`block text-sm font-helvetica mb-2 ${isDark ? 'text-white/80' : 'text-black/80'}`}>
          How thoughtful design, automation, and branding can transform one of the most rigid professional ecosystems.
        </span>
        <p className={`text-xs font-helvetica mb-6 ${isDark ? 'text-white/40' : 'text-black/60'}`}>
        ...
        <section className="mb-10">
          <h2 className="text-lg font-bold mb-4 text-left font-helvetica">
            Product Design
          </h2>
          
          <div className={`space-y-4 text-sm leading-relaxed font-helvetica ${isDark ? 'text-white/70' : 'text-black/80'}`}>
            <p>
        ...
        <section className="mb-10">
          ...
          <div className={`space-y-3 text-sm leading-relaxed font-helvetica ${isDark ? 'text-white/70' : 'text-black/80'}`}>
            <p>500k arr</p>
            <p>4 contracts</p>
          </div>
        </section>
  ...
  return (
    <div className="flex items-center justify-center py-12">
      <p className={`font-helvetica text-sm ${isDark ? 'text-white/60' : 'text-black/60'}`}>
        Article content for {slug} not found
      </p>
    </div>
  )
}
